...
import { deleteRoleplay } from "./actions"
import { useToast } from "@/components/Toast"

export default function DeleteRoleplayButton({ id, title }: { id: string; title: string }) {
  const [confirming, setConfirming] = useState(false)
  const [state, action, pending] = useActionState(
    async (_prev: unknown, formData: FormData) => deleteRoleplay(formData),
    undefined,
  )
  const { show } = useToast()
  ...
  useEffect(() => {
    if (wasPending.current && !pending) {
      if (state?.error) {
        ...
        show("Roleplay eliminado", "success")
        setConfirming(false)
      }
    }
  ...
  return (
    <button
      ...
      aria-label={`Eliminar roleplay ${title}`}

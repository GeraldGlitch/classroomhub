...
export default function DeleteReadingButton({ id, title }: { id: string; title: string }) {
  const [confirming, setConfirming] = useState(false)
  const [state, action, pending] = useActionState(
    async (_prev: unknown, formData: FormData) => deleteReading(formData),
    undefined,
  )
  const { show } = useToast()
  ...
  useEffect(() => {
    if (wasPending.current && !pending) {
      if (state?.error) {
        ...
        show("Lectura eliminada", "success")
        setConfirming(false)
      }
    }
  ...
  return (
    <button
      ...
      aria-label={`Eliminar lectura ${title}`}

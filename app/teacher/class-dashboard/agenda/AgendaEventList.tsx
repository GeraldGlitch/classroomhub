...
export default function AgendaEventList({ events }: { events: AgendaEvent[] }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="mb-3 text-sm font-semibold text-zinc-600">Próximos eventos</h3>
        {upcoming.length === 0 ? (
      ...
      {past.length > 0 && (
        <div>
          <h3 className="mb-3 text-sm font-semibold text-zinc-400">Eventos pasados</h3>
          <div className="space-y-1">
            {past.slice(0, 10).map((event) => (
              <div
                key={event.id}
                className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-zinc-400"
              >
